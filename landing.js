// landing.js (french-cards) — german-cards ile aynı mantık + fr-FR seslendirme
...
// ✅ french-cards için kaç page varsa burayı ayarla
const totalPages = 4;
...
// data'da fr alanı varsa onu key'e koyacağız
const keyOf = (page, fr) => `${page}_${norm(fr)}`;

// ✅ Rastgele modda seçilen sayfalar
const RANDOM_PAGES_KEY = "randomSelectedPages";

// ✅ Random progress
const RANDOM_PROGRESS_KEY = "randomProgress"; // { sig: "1,2", seen: 0, total: 60 }
 ...
 * Beklenen json item alanları: { fr, tr, oku } (oku opsiyonel)
 */
...
function makeCard({ fr, tr, oku, page }, opts = {}) {
  const key = keyOf(page, fr);
  ...
  front.textContent = norm(fr);
  back.innerHTML = `${tr}${oku ? `<br><span>(${oku})</span>` : ""}`;
  tick.textContent = "✔";
  xBtn.textContent = "✘";
...
function prepareRandomDeck(words, selectedPages) {
  ...
  const topKey = keyOf(top.page, top.fr);
  const next = pickFromPool(pool, topKey);
  const nextKey = next ? keyOf(next.page, next.fr) : "";
...
function advanceRandomDeck() {
  ...
  setTimeout(() => {
    ...
    const topKey = randomDeck.top ? keyOf(randomDeck.top.page, randomDeck.top.fr) : "";
